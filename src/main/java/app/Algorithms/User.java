package app.Algorithms;

import app.DTO.RatingDTO;

import java.util.*;

import static java.lang.Math.abs;


public class User {

    private long userid;
    private ArrayList<RatingDTO> ratings;
    private ArrayList<Long> recommendations;

    public User(long userid) {
        this.userid = userid;
        this.ratings = new ArrayList<>();
        this.recommendations = new ArrayList<>();
    }

    public ArrayList<Long> getRecommendations() {
        return recommendations;
    }

    public void setRecommendations(ArrayList<Long> recommendations) {
        this.recommendations = recommendations;
    }

    public void addRating(RatingDTO r) {
        ratings.add(r);
    }

    public long getUserID() {
        return userid;
    }

    public ArrayList<RatingDTO> getRatings() {
        return ratings;
    }

    public void addRecommendation(long i) {
        recommendations.add(i);
    }

    public String writeRecommendations() {
        StringBuilder str = new StringBuilder();
        for (Long i : recommendations) {
            str.append(" ").append(i);
        }
        return str.toString();
    }

    /**
     * This method compares ratings of this and one other user, outputs intersecting and exclusive ratings for second user.
     *
     * @param thisratings       (ratings of first user)
     * @param otherratings      (ratings of second user)
     * @param otherintersection (intersection - ratings shared between first and second user)
     * @param otherexclusives   (exlusives - ratings exclusive to second user)
     */
    private void intersect(ArrayList<RatingDTO> thisratings, ArrayList<RatingDTO> otherratings, ArrayList<RatingDTO> otherintersection, ArrayList<RatingDTO> otherexclusives) {

        boolean otherflag = true;
        if (otherexclusives == null) otherflag = false;
        for (RatingDTO r : otherratings) {
            long movieid = r.getFid();
            boolean flag = false;
            for (RatingDTO p : thisratings) {
                if (movieid == p.getFid()) {
                    flag = true;
                    break;
                }
            }
            if (flag) otherintersection.add(r);
            else if (otherflag) otherexclusives.add(r);
        }
    }

    /**
     * This method calculates spearman rank between for this and one other user.
     *
     * @param other (other user)
     * @return double - spearman rank <-1,1> or -2 in case of insufficient data present.
     */
    public SimilarUser calculateSpearmanRank(User other) {
        // get ratings from both users
        ArrayList<RatingDTO> otherRatings = other.getRatings();
        // get user rated movies intersection (eliminate nonmutual ratings)
        ArrayList<RatingDTO> userSharedRatings = new ArrayList<>();
        ArrayList<RatingDTO> otherSharedRatings = new ArrayList<>();
        ArrayList<RatingDTO> otherExclusiveRatings = new ArrayList<>();
        double spearman = -2;


        // intersect A and B
        intersect(otherRatings, this.ratings, userSharedRatings, null);

        // intersect B and A
        intersect(this.ratings, otherRatings, otherSharedRatings, otherExclusiveRatings);


        // calculate ranks for both users
        Comparator<RatingDTO> comparator = new Comparator<RatingDTO>() {

            @Override
            public int compare(final RatingDTO r1, final RatingDTO r2) {
                // a negative integer, zero, or a positive integer as the first argument is less than, equal to, or greater than the second.
                if (r1.getRating() > r2.getRating()) return 1;
                else if (r1.getRating() < r2.getRating()) return -1;
                else {
                    return Long.compare(r1.getFid(), r2.getFid());
                }
            }
        };

        userSharedRatings.sort(comparator);

        otherSharedRatings.sort(comparator);


        // if the sample is sufficiently large (at least 10 shared movie ratings), calculate spearman
        if (userSharedRatings.size() <= 10) {

            // calculate spearman
            double d = 0;
            for (int i = 0; i < userSharedRatings.size(); i++) {
                RatingDTO r = userSharedRatings.get(i);
                long movieid = r.getFid();
                int j = 0;
                for (; j < otherSharedRatings.size(); j++) {
                    long idatj = otherSharedRatings.get(j).getFid();
                    if (idatj == movieid) break;
                }
                int absval = abs(j - i);
                int sqabs = absval * absval;
                d += sqabs;
            }
            spearman = 1 - (d / (userSharedRatings.size() * ((userSharedRatings.size() * userSharedRatings.size()) - 1)));

        }
        // return spearman
        return new SimilarUser(spearman, other, otherSharedRatings, otherExclusiveRatings);
    }

    /**
     * This method calculates recommended movies for this user based on
     *
     * @param users (ArrayList with data for cf - ratings paired with thier users)
     */
    public void recommend(ArrayList<User> users, Settings settings) {

        // find x most similar users
        Comparator<SimilarUser> cmp = new Comparator<SimilarUser>() {
            @Override
            public int compare(SimilarUser o1, SimilarUser o2) {
                return -1 * Double.compare(o1.getSpearmanRank(), o2.getSpearmanRank());
            }
        };
        PriorityQueue<SimilarUser> similarQueue = new PriorityQueue<>(10, cmp);

        for (User u : users) {
            SimilarUser su = calculateSpearmanRank(u);
            if (similarQueue.size() < settings.getNUMBER_OF_SIMILAR_USERS()) similarQueue.add(su);
            else if (su.getSpearmanRank() > similarQueue.peek().getSpearmanRank()) {
                similarQueue.poll();
                similarQueue.add(su);
            }
        }
        // aggregate their ratings and find best recommendations
        ArrayList<SimilarUser> similarUsers = new ArrayList<>();
        while (!similarQueue.isEmpty()) {
            similarUsers.add(similarQueue.poll());
        }

        HashMap<Long, RecommendedMovie> aggregatedRatings = new HashMap<>();

        for (SimilarUser u : similarUsers) {
            for (RatingDTO r : u.getExclusive_ratings()) {
                if (aggregatedRatings.containsKey(r.getFid())) continue;
                long movieid = r.getFid();
                double rating = r.getRating();
                int ctr = 1;
                long usrid = u.getUser().getUserID();
                for (SimilarUser i : similarUsers) {
                    if (i.getUser().getUserID() == usrid) continue;
                    for (RatingDTO a : i.getExclusive_ratings()) {
                        if (a.getFid() == movieid) {
                            rating += a.getRating();
                            ctr++;
                            break;
                        }
                    }
                }
                // calculate average rating
                double aggrating = rating / ctr;

                // TODO experiment with required similarity rate (currently 40%)
                if (ctr > settings.getNUMBER_OF_REQUIRED_MATCHES())
                    aggregatedRatings.put(movieid, new RecommendedMovie(movieid, aggrating)); //
            }
        }

        Iterator it = aggregatedRatings.values().iterator();
        ArrayList<RecommendedMovie> recommendedMovies = new ArrayList<>();

        while (it.hasNext()) {
            recommendedMovies.add((RecommendedMovie) it.next());
        }

        Comparator<RecommendedMovie> recommendedMovieComparator = new Comparator<RecommendedMovie>() {
            @Override
            public int compare(RecommendedMovie o1, RecommendedMovie o2) {
                return Double.compare(o1.aggregatedRating, o2.aggregatedRating);
            }
        };
        recommendedMovies.sort(recommendedMovieComparator);
        this.recommendations.clear();
        for (int i = 0; i < settings.getNUMBER_OF_RECOMMENDATIONS(); i++) { // TODO specify number of recommended movies
            if (i < recommendedMovies.size()) this.recommendations.add(recommendedMovies.get(i).movieID);
        }

        //System.out.println("Added " + recomos.size() + " recommendations")

    }
}
